...
class GameOfLife:
                    ...
                    if self.cell_list[i][j] == 0:
                        if colors_distribution[0] == 5:
                            ...
                            if max_color == 3:
                                ...
                                self.score[col]['converted'] += 1
                            else:
                                temporary_matrix[i][j] = 1
                                self.score[1]['result'] += 1
                                self.score[1]['converted'] += 1
                        else:
                        ...
                        if colors_distribution[0] in [5, 6]:
                            ...
                            temporary_matrix[i][j] = col
                            self.score[col]['result'] += 1
                            if col != self.cell_list[i][j]:
                                self.score[col]['converted'] += 1
                        else:
                            temporary_matrix[i][j] = 0
                            self.score[0]['result'] += 1
                            self.score[0]['converted'] += 1
        if k == 1:
            temporary_matrix = [[0] * self.cell_width for i in range(self.cell_height)]
            for i in range(self.cell_height):
                for j in range(self.cell_width):
    ...
    def run_war(self):
        ...
        if running:
            ...
            if running:
                self.create_cell_list(n=s, dop="blue", iswar = True)
                self.create_cell_list(n=c, dop = "red", iswar = True)
                s = 0
                c = 0
            ...
            if running:
                ...
                print("Second player captured: ", points21, points22, points23, points24, points25)
                if points11 > 0 and points12 > 0 and points13 > 0 and points14 > 0 and points15 > 0:
                    print('Player #2 wins 3d round!')
                    winner = 1
                elif points21 > 0 and points22 > 0 and points23 > 0 and points24 > 0 and points25 > 0:
                    print('Player #1 wins 3d round!')
                    winner = 2
                else:
                    print('War, war never changes')
                    winner = 3
            ...
            while final:
                if winner == 1:
                    myImage1 = pygame.image.load('images\\bluewin.jpg')
                if winner == 2:
                    myImage1 = pygame.image.load('images\\redwin.jpg')
                if winner == 3:
                    myImage1 = pygame.image.load('images\\draw.jpg')
                myRect1 = (0, 0, 640, 480)
                self.screen.blit(myImage1, myRect1)
                pygame.display.flip()
                clock.tick(self.speed)
    ...
    def run_economics(self):
        ...
        while running and self.counter < 40:
            ...
            pygame.display.flip()
            clock.tick(self.speed)
            if (self.counter + 1) % 10 == 0 and (self.counter + 1) < 40:
                self.create_cell_list(n=self.score[2]['converted'] // 40, dop="blue")
                self.create_cell_list(n=self.score[3]['converted'] // 40, dop="red")

        if running:
            self.counter = 0
            results = [i['converted'] for i in self.score[2:]]
            if not max(results) == min(results):
    ...
    def run_religion(self):
        ...
        while running and self.counter < 30:
            ...
            pygame.display.flip()
            clock.tick(self.speed)
            if (self.counter + 1) % 10 == 0 and (self.counter + 1) < 30:
                self.create_cell_list(n=self.score[2]['result'] // 5, dop="blue")
                self.create_cell_list(n=self.score[3]['result'] // 5, dop="red")
    ...
    def create_cell_list(self, randomize=False, b='black', n=40, dop = "", iswar = False):
        if n == 40:
        ...
        if randomize:
            ...
            while count < n:
                ...
                if not(istip) and dop!="":
                    f1 = pygame.font.Font(None, 36)
                    s = dop + ' player have ' + str(n - count) +   ' cells left'
                    if dop == "red":
                        text1 = f1.render(s, 1, (180, 0, 0))
                    if dop == "blue":
                        text1 = f1.render(s, 1, (0, 0, 180))
                    self.screen.blit(text1, (150, 225))
                    istip = True
                    pygame.display.flip()
